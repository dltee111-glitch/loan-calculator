...
with tab1:
    ...
    if method == "원리금 균등 상환":
        monthly_payment = df_calc.iloc[0]['상환 원금'] + df_calc.iloc[0]['이자']
        col_r1.metric("월 예상 상환액", f"{int(monthly_payment):,} 원")
    else:
        col_r1.info("원금 균등 방식은 매월 상환액이 감소합니다.")
    
    col_r2.metric("총 이자 비용", f"{int(df_calc['이자'].sum()):,} 원")
...
with tab3:
    ...
    with c_a:
        rate1 = st.number_input("현재 금리 (%)", value=rate, step=0.1)
    with c_b:
        rate2 = st.number_input("비교 금리 (%)", value=rate-1.0, step=0.1)
